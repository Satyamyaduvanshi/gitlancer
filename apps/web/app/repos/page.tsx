...
import { Search, Plus, FolderGit2, ArrowRight } from 'lucide-react';
  ...
  return (
    <DashboardLayout>
      ...
      >
        ...
        {/* 🔍 Search Bar */}
        <motion.div variants={itemVariants} className="relative w-full mb-8 group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading ? (
            ...
            <motion.div variants={itemVariants} className="col-span-full py-20 text-center bg-[#111111]/50 border border-white/5 rounded-[2rem]">
              <FolderGit2 size={48} className="text-white/20 mb-4 mx-auto" />
            ...
            filteredVaults?.map((vault: any) => (
              <motion.div 
                variants={itemVariants} // 🛡️ Each card gets the item variant to create the stagger effect
                key={vault.id} 
                onClick={() => router.push(`/repos/${encodeURIComponent(vault.repositoryFullName)}`)}
                className="bg-[#111111] border border-white/5 p-7 rounded-[2rem] shadow-lg cursor-pointer hover:border-persimmon/30 hover:shadow-[0_15px_40px_rgba(252,76,2,0.1)] hover:-translate-y-1 transition-all duration-500 group flex flex-col justify-between min-h-[200px] relative overflow-hidden"
              >
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <div className="p-2.5 bg-white/5 rounded-xl text-white/50 group-hover:text-white group-hover:bg-white/10 transition-colors relative z-10">
                    ...
                    <h3 className="font-bold text-white text-lg truncate tracking-tight group-hover:text-persimmon transition-colors relative z-10">
                      {vault.repositoryFullName.split('/')[1] || vault.repositoryFullName}
                    </h3>
                  </div>
                  
                  <div className="bg-white/[0.02] rounded-xl p-3 border border-white/5 mb-2 relative z-10">
                ...
                <div className="flex justify-between items-center text-sm mt-6 pt-4 border-t border-white/5 relative z-10">
                ...
                <div className="absolute right-6 top-8 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500 z-10">
